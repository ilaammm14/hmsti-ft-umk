...
const DEPARTEMEN = ['Minat dan Bakat', 'Kerohanian', 'Pendidikan dan Kaderisasi', 'Kesektretariatan']

interface Props { internal: Anggota[] }

export default function InternalChart({ internal }: Props) {
  const [activeDept, setActiveDept] = useState<string | null>(null)

  const j = (a: Anggota) => a.jabatan.toLowerCase()
  const pemimpin = internal.find(a =>
    j(a).includes('ketua') || j(a).includes('kordinator') || j(a).includes('koordinator')
  )

  const getAnggotaDept = (dept: string) => {
    const deptLower = dept.toLowerCase()
    return internal.filter(a => {
  ...
  return (
    <div className="w-full">
      ...
      {/* Tombol departemen — scroll horizontal di mobile */}
