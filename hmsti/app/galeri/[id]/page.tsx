...
import { ArrowLeft, ChevronLeft, ChevronRight, Images, X, ZoomIn } from 'lucide-react'
    ...
    <div className="fixed inset-0 z-50 bg-black/95 flex flex-col" onClick={onClose}>
      ...
      <div className="flex-1 flex items-center justify-center relative px-12 min-h-0"
        ...
        {images.length > 1 && (
          <>
            <button onClick={prev}
              className="absolute left-2 top-1/2 -translate-y-1/2 bg-white/10 hover:bg-white/20 text-white rounded-full p-3 transition-all">
              <ChevronLeft size={24} />
            </button>
            <button onClick={next}
              className="absolute right-2 top-1/2 -translate-y-1/2 bg-white/10 hover:bg-white/20 text-white rounded-full p-3 transition-all">
          ...
          {images.map((img, i) => (
            <button key={img.id} onClick={() => setIdx(i)}
              className={`shrink-0 w-14 h-14 rounded-lg overflow-hidden border-2 transition-all ${
                i === idx ? 'border-amber-500 opacity-100' : 'border-transparent opacity-40 hover:opacity-70'
              }`}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {images.length > 0 ? (
            <div className="relative mb-6">
              {/* Gambar utama — klik untuk perbesar */}
              <div className="relative aspect-video sm:aspect-[4/3] rounded-2xl overflow-hidden bg-gray-800 cursor-zoom-in group"
