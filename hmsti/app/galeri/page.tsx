          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
              {filtered.map(item => {
                ...
                return (
                  <button key={item.id} onClick={() => router.push(`/galeri/${item.id}`)}
                    ...
                    <div className="p-4">
                      <h3 className="font-semibold text-white group-hover:text-amber-400 transition-colors text-sm sm:text-base">{item.judul}</h3>
                      {item.deskripsi && (
                      ...
                      {count > 1 && (
                        <div className="flex gap-1.5 mt-3">
                          {item.images.slice(1, 4).map(img => (
                            <div key={img.id} className="w-10 h-10 rounded-lg overflow-hidden shrink-0">
                              {/* eslint-disable-next-line @next/next/no-img-element */}
                              <img src={img.image_url} alt="" className="w-full h-full object-cover" loading="lazy" />
                            </div>
                          ))}
                          {count > 4 && (
                            <div className="w-10 h-10 rounded-lg bg-gray-800 border border-gray-700 flex items-center justify-center text-xs text-gray-400 shrink-0">
                              +{count - 4}
                            </div>
                          )}
                        </div>
